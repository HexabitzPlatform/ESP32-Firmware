...
static const ble_uuid_t *remote_svc_uuid = BLE_UUID128_DECLARE(0x90, 0x89, 0x81,
		0x40, 0xec, 0xc4, 0x96, 0xb7, 0xed, 0x45, 0x63, 0xcc, 0x1d, 0x46, 0xdb,
		0xf9);

/*** The UUID of the subscribable chatacteristic ***/
static const ble_uuid_t *remote_chr_uuid = BLE_UUID128_DECLARE(0xe2, 0xa2, 0xe5, 0x9c, 0x72, 0x24, 0xa7, 0x99,
        0xc6, 0x46, 0x50, 0x69, 0x11, 0x65, 0xa7, 0xb8);
///*** The UUID of the service containing the subscribable characterstic ***/
//static const ble_uuid_t *remote_svc_uuid = BLE_UUID128_DECLARE(0x2d, 0x71, 0xa2,
//		0x59, 0xb4, 0x58, 0xc8, 0x12, 0x99, 0x99, 0x43, 0x95, 0x12, 0x2f, 0x46,
//		0x59);
//
///*** The UUID of the subscribable chatacteristic ***/
//static const ble_uuid_t *remote_chr_uuid = BLE_UUID128_DECLARE(0x00, 0x00, 0x00,
//		0x00, 0x11, 0x11, 0x11, 0x11, 0x22, 0x22, 0x22, 0x22, 0x33, 0x33, 0x33,
//		0x33);
uint8_t z = 10;
 ...
 **/
int blecent_on_custom_read(uint16_t conn_handle,
		const struct ble_gatt_error *error, struct ble_gatt_attr *attr,
		void *arg) {
	MODLOG_DFLT(INFO,
			"Read complete for the subscribable characteristic; " "status=%d conn_handle=%d",
 ...
 **/
int blecent_on_custom_write(uint16_t conn_handle,
		const struct ble_gatt_error *error, struct ble_gatt_attr *attr,
		void *arg) {
	...
	MODLOG_DFLT(INFO,
			"Write to the custom subscribable characteristic complete; " "status=%d conn_handle=%d attr_handle=%d\n",
	...
	if (rc != 0) {
		MODLOG_DFLT(ERROR,
				"Error: Failed to read the custom subscribable characteristic; " "rc=%d\n",
				rc);
		goto err;
	}
...
int blecent_on_custom_subscribe(uint16_t conn_handle,
		const struct ble_gatt_error *error, struct ble_gatt_attr *attr,
		void *arg) {
	...
	MODLOG_DFLT(INFO,
			"Subscribe to the custom subscribable characteristic complete; " "status=%d conn_handle=%d",
			error->status, conn_handle);
...
//    write_data(rr);
	rc = ble_gattc_write_flat(conn_handle, chr->chr.val_handle, &value,
			sizeof(value), blecent_on_custom_write, NULL);
	if (rc != 0) {
		MODLOG_DFLT(ERROR,
 ...
 **/
void blecent_custom_gatt_operations(const struct peer *peer) {
	...
	rc = ble_gattc_write_flat(peer->conn_handle, dsc->dsc.handle, value,
			sizeof(value), blecent_on_custom_subscribe, NULL);
	if (rc != 0) {
		MODLOG_DFLT(ERROR,
 ...
 */
int blecent_on_write(uint16_t conn_handle, const struct ble_gatt_error *error,
 ...
 */
int blecent_on_read(uint16_t conn_handle, const struct ble_gatt_error *error,
		struct ble_gatt_attr *attr, void *arg) {
	...
	if (chr == NULL) {
		MODLOG_DFLT(ERROR,
				"Error: Peer doesn't support the Alert " "Notification Control Point characteristic\n");
		goto err;
	}
 ...
 */
void blecent_read_write_subscribe(const struct peer *peer) {
	const struct peer_chr *chr;
	int rc;
...
void blecent_on_disc_complete(const struct peer *peer, int status, void *arg) {
	...
	if (status != 0) {
		...
		MODLOG_DFLT(ERROR,
				"Error: Service discovery failed; status=%d " "conn_handle=%d\n",
	 ...
	 */
	MODLOG_DFLT(INFO,
			"Service discovery complete; status=%d " "conn_handle=%d\n", status,
			peer->conn_handle);
...
int blecent_should_connect(const struct ble_gap_disc_desc *disc) {
	...
	if (strlen(CONFIG_EXAMPLE_PEER_ADDR)
			&& (strncmp(CONFIG_EXAMPLE_PEER_ADDR, "ADDR_ANY",
					strlen("ADDR_ANY")) != 0)) {
...
void blecent_connect_if_interesting(void *disc) {
	...
	rc = ble_gap_connect(own_addr_type, addr, 30000, NULL, blecent_gap_event,
			NULL);
	if (rc != 0) {
		MODLOG_DFLT(ERROR,
				"Error: Failed to connect to device; addr_type=%d " "addr=%s; rc=%d\n",
...
int write_data(uint8_t *data) {
	...
	rc = ble_gattc_write_flat(conn, chr->chr.val_handle, Bledata,
			sizeof(Bledata), blecent_on_custom_write, NULL);
	if (rc != 0) {
		MODLOG_DFLT(ERROR,
				"Error: Failed to subscribe to characteristic; " "rc=%d\n", rc);
	}
...
int read_data() {
	...
	for (int i = 0; i < 18; ++i) {
		ESP_LOGI("BleReadDataClient", "BleReadDataClient = %d",
				BleReadDataClient[i]);
	}
	if (rc != 0) {
		MODLOG_DFLT(ERROR,
...
void BLECLient(char *clientname) {
	...
	rc = ble_svc_gap_device_name_set(clientname);
	assert(rc == 0);
